...
export class EditComponent implements OnInit {
  ...
  updateInfo(children) {
    ...
    // Url to API
    var url = "http://18.216.142.10:3000/api/" + this.profileType + "/" + this.user;
  ...
  ngOnInit() {
    ...
    if (sessionStorage.getItem("accountType") == "employer") {
      ...
      var url = "http://18.216.142.10:3000/api/Employer/" + this.user;
    ...
    this.http.post(url, data).subscribe(
      data => {
        ...
        this.http.get("http://18.216.142.10:3000/api/Employer/" + localStorage.getItem("CognitoIdentityServiceProvider.7tvb9q2vkudvr2a2q18ib0o5qt.LastAuthUser")).subscribe(
          emp_data => {
            this.http.get("http://18.216.142.10:3000/api/Job/" + sessionStorage.getItem("fromJob")).subscribe(
              job_data => {
                var mailData = {
                  to: this.email,
